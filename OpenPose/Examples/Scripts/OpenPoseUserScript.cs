...
namespace OpenPose.Example {
    ...
    public class OpenPoseUserScript : MonoBehaviour {
        ...
        public string[] index2name = new string[] { "Nose", "Neck", "RShoulder", "RElbow", "RWrist", "LShoulder", "LElbow", "LWrist", "MidHip", "RHip", "RKnee", "RAnkle", "LHip", "LKnee", "LAnkle", "REye", "LEye", "REar", "LEar", "LBigToe", "LSmallToe", "LHeel", "RBigToe", "RSmallToe", "RHeel", "Background" };
        ...
        private void Update() {
            ...
            if (OPWrapper.OPGetOutput(out datum)){ // true: has new frame data
                ...
                else
                {
                    ...
                    if (datum.frameNumber % 5 == 0)
                    {
                        /*
                        Debug.Log(" --- GetSize start ---");
                        foreach (var size in datum.poseKeypoints.GetSize())
                        {
                            Debug.Log("GetSize :" + size);
                        }
                        Debug.Log(" --- GetSize end ---");
                        */
                        /*
                        Debug.Log(" --- output start ---");
                        for (int index = 0; index < datum.poseKeypoints.GetSize()[1]; index++) //25
                        {
                            if (index == 3 || index == 4 || index == 6 || index == 7)
                                Debug.Log(index2name[index] + " :" + datum.poseKeypoints.Get(0, index, 0) + " " +
                                    datum.poseKeypoints.Get(0, index, 1) + " " + datum.poseKeypoints.Get(0, index, 2));
                        }
